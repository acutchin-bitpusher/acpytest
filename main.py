...
def hello():
  ...
  response = ""
  response += "Hello from acpytest w/ api integration! <br/> \n"
  response += "version: " + version + " <br/> \n"
  response += "<br/> \n"
#  response += db_config_html(environ)
#  response += sqlalchemy_db_test_html(environ,app)
  api_tables_uri = "http://" + str(environ.get('API_ENDPOINT')) + "/tables"
  api_response = requests.get(api_tables_uri)
  response += api_response.text + "<br/> \n"
  response += "<br/> \n"
